<template>
    <PageLayout :title="'Coffee Details'">
            ...
            <div class="flex flex-col md:flex-row -mx-4">
                ...
                <div class="md:flex-1 px-4 sm:justify-center">
                    <h2 class="mb-2 leading-tight tracking-tight font-bold text-gray-800 text-2xl md:text-3xl">
                        {{ coffee.name }}</h2>
                    <p class="text-gray-500 text-sm">By <a href="#" class="text-[#7c4e29] hover:underline">ABC
                        Company</a>
                    </p>
                    ...
                    <div class="flex py-4 space-x-4 pt-16">
                        <button
                            v-if="!coffee.isRemoved"
                            class="mr-3 text-gray-900 border-2 border-transparent border-gray-900 py-2 px-5 text-md font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#7c4e29]"
                            type="submit"
                            @click="editCoffee"
                        >Edit
                        </button>
                        <button
                            ...
                            type="submit"
                            @click="removeCoffee"
                        >{{ coffee.isRemoved ? 'Restore' : 'Remove' }}
                        </button>
                    </div>
                </div>
            </div>
...
const removeCoffee = () => {
    if (coffee.value.id) {
        if (coffee.value.isRemoved) {
            coffeeStore.restoreCoffee(coffee.value.id);
        } else {
